...
def extract_show(App,f,config_file):
    ...
    with open(nf) as json_file:
        ...
        if earnings>0:
            show_data=ndate+' ' +data['show']+'\n'+start+' to ' +end+' with '+str(lunch)+' lunche(s)\n'+str(worked_hours)+'*'+str(rate)+' earned: '+str(earnings)
        else:
            show_data=ndate+' ' +data['show']+'\n'+start+' to ' +end+' with '+str(lunch)+' lunche(s)\n'



        return show_data,(earnings),(overtime),(regular_hours),int(worked_hours),data['date']
...
def extract_pos(App,config_file,pos):
    ...
    nf=(os.path.join(config_file,'pos', pos,))
    try:
