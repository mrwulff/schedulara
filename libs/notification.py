...
import libs.notification as _notification
import sys
import uuid
import os
import json
...
def _get_soundlib_path():
    ...
    NSUserDomain = 1
    library_dir_url = fm.URLForDirectory_inDomain_appropriateForURL_create_error_(
        NSLibraryDirectory, NSUserDomain, None, True, None
    )
...
def get_scheduled():
    ...
    def handler_f(_, requests_ptr):
        if requests_ptr:
            requests = ObjCInstance(requests_ptr)
            for r in requests:
                identifier = str(r.identifier())
                identifiers.append(identifier)
        sema.release()

    handler = ObjCBlock(handler_f, restype=None, argtypes=[c_void_p, c_void_p])
    CENTER.getPendingNotificationRequestsWithCompletionHandler_(handler)
    sema.acquire()
    sema.release()
    return identifiers
                ...
                for i, a in enumerate(cat_actions):
                    ...
                    if action_url and not force_foreground:
                        if not action_url.startswith(
                            "pythonista:"
                        ) and not action_url.startswith("pythonista3:"):
                    ...
                    if a.get("text_input"):
                        ...
                        un_action = (
                            UNNotificationAction.actionWithIdentifier_title_options_(
                                button_id, button_title, action_options
                            )
                        )
...
def _process_attachments(attachments):
    ...
    for attachment in attachments:
        ...
        try:
            ...
            raise ValueError(
                "Invalid attachment or unreadable file: %s" % (attachment,)
            )
...
def _parse_location_trigger(trigger):
    ...
    region = (
        CLCircularRegion.alloc()
        .initWithCenter_radius_identifier_(center, radius, region_id)
        .autorelease()
    )
    region.setNotifyOnEntry_(trigger.get("entry", True))
    region.setNotifyOnExit_(trigger.get("exit", True))
    un_trigger = UNLocationNotificationTrigger.triggerWithRegion_repeats_(
        region, trigger.get("repeats", False)
    )
    if un_trigger:
        import location
...
def _parse_trigger(trigger, delay):
    ...
    if trigger is not None:
        if trigger is not None:
            ...
            if date_comps:
                un_trigger = UNCalendarNotificationTrigger.triggerWithDateMatchingComponents_repeats_(
                    date_comps, repeats
                )
                if not un_trigger or not un_trigger.nextTriggerDate():
                    raise ValueError(
                        "Invalid date components for calendar trigger (in the past?)"
                    )
        ...
        un_trigger = UNTimeIntervalNotificationTrigger.triggerWithTimeInterval_repeats_(
            delay, False
        )
    return un_trigger
...
def _process_sound(sound_name, content):
    ...
    if sound_file_path:
        ...
        try:
            ...
            raise ValueError(
                "Invalid sound_name or unreadable file: %s" % (sound_name,)
            )
...
if __name__ == "__main__":
    ...
    schedule(
        "Test",
        title="Title",
        sound_name="game:Beep",
        attachments=[ui.Image("test:Mandrill")],
    )
